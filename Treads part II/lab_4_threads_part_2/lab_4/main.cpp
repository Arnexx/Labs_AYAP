...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pszCmdParam, int nCmdShow)
{
	...
	hWnd = CreateWindow(szClassName,(LPSTR)"Lab_4",WS_OVERLAPPEDWINDOW,0,0,1300,700,NULL,NULL,hInstance,NULL);
...
LRESULT CALLBACK WndProc( HWND hWnd, UINT msg, UINT wParam, LONG lParam)
{
	...
	{
	case WM_CREATE:
		hEvent = CreateEvent(0, true, false, "Rotation");  //cоздание события
		break;
	case WM_INITPARAMS:
		{
			RECT rect;
			...
			params->H_event = hEvent;
			params->RotationEvent = 0;
			...
			{
				temp.x = rand()%1000+100;
				temp.y = rand()%500+100;
				if(itr % 3 == 1)
					mVector[itr] = new Square(temp, rand()%200+30, M_PI/(float)(rand()%12),
					                  M_PI/(float)(rand()%70+35), rand()%2, rand()%5+2, rand()%256, rand()%256, rand()%256);
			...
			for(int itr = 0; itr < MAXSH; itr++)
				params->mShapes[itr] = mVector[itr];
			for (int itr = 0 ; itr < threadsCounter -1; itr++)
			{
				params->shapes_in_thread.push_back(MAXSH/threadsCounter);
			}
			params->onDraw = false;
	...
	case  WM_TIMER :
		{  
			...
			{
				params->onDraw = true;
			for (int itr = 0; itr < lastShapes; itr++)
			{
